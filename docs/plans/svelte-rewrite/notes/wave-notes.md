...
- `durablefunctionsmonitor.svelte/.npmrc` has `engine-strict=false`, not `true` as E0-S1-T1 step 6 asks. The README pins `jsdom` 30.0.1, whose `engines` is `^22.22.2 || ^24.15.0 || >=26.0.0` (and its `undici` 8.10.2 wants `>=22.19.0`), while the reference toolchain is Node 22.18.0; with `engine-strict=true` `npm ci` hard-fails with EBADENGINE. jsdom 30.0.1 runs fine on 22.18.0. **Plan owner decision pending:** bump the reference Node to >= 22.22.2 (then restore `engine-strict=true`) or relax the jsdom pin. E12's CI job must use a Node that satisfies jsdom.
